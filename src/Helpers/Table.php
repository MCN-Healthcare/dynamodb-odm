...
class Table
{
    ...
    )
    {
        ...
        foreach ($keys as $key) {
            $keyItem = $keyIsTyped ? Item::createFromTypedArray($key) :
                Item::createFromArray($key, $this->attributeTypes);
            $req     = $keyItem->getData();
            $reads[] = $req;
    ...
    public function get(array $keys, $is_consistent_read = false, $projectedFields = [])
    {
        ...
        if ($result['Item']) {
            $item = Item::createFromTypedArray((array)$result['Item']);
    ...
    public function set(array $obj, $checkValues = [])
    {
        ...
        if ($checkValues) {
            ...
            $typedCheckValues = Item::createFromArray($checkValues)->getData();
            $casCounter       = 0;
                                    ...
                                    $maxDelay = 15000)
    {
        ...
        foreach ($objs as $obj) {
            $item = $objIsTyped ? Item::createFromTypedArray($obj) :
                Item::createFromArray($obj, $this->attributeTypes);
            if ($isPut) {
                $req = [
                    "PutRequest" => [
                        "Item" => $item->getData(),
